...
export class DocumentProcessingService {
   ...
   */
  async analyzeDocument(fileId: string, options: any = {}): Promise<AnalysisResult> {
    try {
      // Check cache first
      ...
      throw new AppError(`Failed to analyze document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  /**
   * Process document (analyze and optionally save)
   */
  async processDocument(fileId: string, options: ProcessDocumentOptions = {}): Promise<AnalysisResult | { jobId: string }> {
    try {
      if (options.async) {
        // Start async processing
        const jobId = this.generateJobId();
        this.startAsyncProcessing(jobId, fileId, options);
        return { jobId };
      }
   ...
   */
  async saveDocument(documentData: CreateDocumentRequest): Promise<DatabaseDocument> {
    try {
      // Validate document data
      const validation = this.validationService.validateDocumentForSave(documentData);
      if (!validation.isValid) {
        throw new AppError(`Validation failed: ${validation.errors.join(', ')}`);
      }
      ...
      return await prisma.$transaction(async (tx) => {
        ...
        const processedEntities = await Promise.all(
          documentData.entities.map(async (entityData) => {
            ...
            if (!existingEntity) {
              existingEntity = await this.entityRepository.create(entityData, tx);
            }
      ...
      throw new AppError(`Failed to save document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async convertToImage(file: File): Promise<string> {
    try {
      ...
      throw new AppError(`Failed to convert document to image: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
