...
export class StorageService {
  ...
  private initializeAuth(): void {
    try {
      if (!fs.existsSync(config.GOOGLE_CREDENTIALS_PATH)) {
        throw new AppError(
          `Google credentials file not found at ${config.GOOGLE_CREDENTIALS_PATH}. ` +
          'Please ensure GOOGLE_CREDENTIALS_PATH is set correctly or the credentials file exists.',
          500
        );
      }

      const credentials = JSON.parse(fs.readFileSync(config.GOOGLE_CREDENTIALS_PATH, 'utf8'));
      ...
      throw new AppError('Failed to initialize Google Drive authentication');
    }
  }
  ...
  private initializeDrive(): void {
    try {
      ...
      throw new AppError('Failed to initialize Google Drive API');
    }
  }
